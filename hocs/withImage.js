    ...
    const EnhancedComponent = (props)=>{
        ...
        const saveFileToState = (file)=>{
            if(name==='product'){
                ...
                dispatch(showSuccessAlert('Images has been uploaded successfully'));
            }else{
                const img = { id: `${file.name}-${Date.now()}`, image: file };
                setSelectedImages([img]);
                dispatch(showSuccessAlert(`Image has been uploaded successfully`));
            }
        }
            ...
            if(name==='product'){
                ...
                for(let f of file){
                    if(!f.type.startsWith('image')){
                        dispatch(showErrorAlert('Selected files are not of type image, please select images only'));
                        return;
                    }
                }
                ...
                for(let i in file){
                    ...
                    reader.onload = (e)=>{
                        ...
                        image.onload = (e)=>{
                            ...
                            if(height!==h || width!==w){
                                dispatch(showErrorAlert(`All images size should be ${h}*${w}`));
                                return;
                            }
                            else{
                                if(i==file.length-1){
                ...
                if(!file.type.startsWith('image')){
                    dispatch(showErrorAlert('Selected file is not of type image, please select image only'));
                    return;
                }
                ...
                reader.onload = (e)=>{
                    ...
                    image.onload = (e)=>{
                        ...
                        const width = e.target.width;
                        if(height!==h || width!==w){
                            dispatch(showErrorAlert(`Image size should be ${h}*${w}`));
                            return;
                        }
                        else{
                            saveFileToState(file);

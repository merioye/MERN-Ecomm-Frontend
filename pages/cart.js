...
import { useTheme, Box, Grid, Typography, Divider, Button, TextField } from "@mui/material";
        ...
        >
            <Grid item xs={8}>
                ...
                >
                    {
                        cartItems.map((item)=>{
                            return <CartPageProduct key={item.product._id} item={item}/>
                        })
                    }
                </Grid>
            </Grid>
            <Grid item xs={3.75}>
                ...
                >
                    ...
                    >
                        <Grid item>
                            <Typography variant='body1' sx={{ fontSize: '14px', color: 'text.light' }}>Total:</Typography>
                        </Grid>
                        <Grid item>
                            <Typography variant='h3' sx={{ fontSize: '18px', fontWeight: 600 }}>${total}.00</Typography>
                        </Grid>
                    </Grid>  
                    ...
                    >
                        <Grid item>
                            <Typography variant='body1' sx={{ fontSize: '14px', fontWeight: 600 }}>Additional Comments</Typography>
                        </Grid>
                        <Grid item>
                            <Typography variant='h3' sx={style.note}>Note</Typography>
                        </Grid>
                    </Grid> 
                    ...
                    <Box mb='16px'>
                        <TextField
                            ...
                            autoComplete='off'
                            InputProps={{
                                style: {...theme.userInput, height: '150px', alignItems: 'flex-start' }
                            }}
                            value={note}
                            onChange={(e)=> dispatch(setNoteValue(e.target.value))}
                            multiline={true}
                            maxRows={6}
                        />
                    </Box>
...
const CartPage = ( { cartItems, voucher } ) => {
    ...
    const { isCartItemsFetched, cartItems:items, note, voucher:cartVoucher } = useSelector((state)=>state.cart);
    const dispatch = useDispatch();
    ...
    return (
        <Box sx={style.container}>
            {
                cartItems.length && (isCartItemsFetched ? Boolean(items.length) : true)
                ?
                isCartItemsFetched
                    ?
                    <CartData cartItems={items} voucher={cartVoucher ? cartVoucher : voucher} note={note}/>
                    :
                    <CartData cartItems={cartItems} voucher={cartVoucher ? cartVoucher : voucher} note={note}/>
                :
                <Grid
                    container
                    flexDirection='column'
                ...
                >
                    ...
                    <Grid item>
                        <Typography variant='h2' sx={{ fontSize: '20px', color: 'text.primary', fontWeight: 600 }}>Your Cart is empty</Typography>
                    </Grid>
                </Grid>
            }
        </Box>

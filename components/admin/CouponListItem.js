...
const CouponListItem = ({ coupon }) => {
    ...
    const handleDelete = () => {
        ...
        dispatch(
            deleteCoupon(coupon._id, setShowDeleteLoader, pageCount, router)
        );
    };
    ...
    return (
        <TableRow sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
            ...
            >
                {new Date(coupon.createdAt).toDateString().slice(4)}
            </TableCell>
            <TableCell
                align="center"
                sx={theme.adminTableCellText}
                style={{ minWidth: "120px" }}
            >
                {new Date(coupon.validity).toDateString().slice(4)}
            </TableCell>
            <TableCell
                align="center"
                sx={theme.adminTableCellText}
            ...
            <TableCell align="center" sx={theme.adminTableCellText}>
                <Box
                    sx={
                        new Date().getTime() >=
                        new Date(coupon.validity).getTime()
                            ? {

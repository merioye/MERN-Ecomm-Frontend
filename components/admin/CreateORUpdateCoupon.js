...
const CreateORUpdateCoupon = ({
    ...
    const [values, setValues] = useState({
        ...
        validity: data
            ? new Date(data.validity).toISOString().split("T")[0]
            : "",
    ...
    const handleFormSubmit = (e) => {
        ...
        if (id) {
            if (
                ...
                values.couponCode === data.couponCode &&
                values.validity === data.validity &&
                values.discountPercentage === data.discountPercentage &&
                values.minimumAmount === data.minimumAmount &&
            ...
            if (new Date(values.validity).getTime() <= new Date().getTime()) {
                dispatch(
                    showErrorAlert(
                        "Validity Date/Time must be greater than current Date/Time"
                    )
                );
                return;
            }
            if (
                ...
                dispatch(
                    showErrorAlert("Please enter a valid discount percentage")
                );
                return;
            }
            if (values.minimumAmount <= 0) {
                dispatch(
                    showErrorAlert(
                        "Minimum Purchase amount should be greater than 0"
                    )
                );
                return;
            }
                    ...
                    >
                        ...
                        >
                            ...
                            <Typography
                                ...
                                style={
                                    isDragActive ? { color: "#4E97FD" } : null
                                }
                            >
                                {isDragActive ? (
                                    <>Drop banner here...</>

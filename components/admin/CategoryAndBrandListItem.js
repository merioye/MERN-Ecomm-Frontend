...
const CategoryAndBrandListItem = ({ item, index, name }) => {
    ...
    const handleDelete = () => {
        ...
        if (name === "Brand") {
            dispatch(
                deleteBrand(item._id, setShowDeleteLoader, pageCount, router)
            );
        } else {
            dispatch(
                deleteCategory(item._id, setShowDeleteLoader, pageCount, router)
            );
        }
    };
                ...
                <Stack direction="row">
                    <IconButton
                        ...
                        onClick={() =>
                            router.push(
                                `/admin/${
                                    name === "Brand" ? "brands" : "categories"
                                }/${item._id}?page=${pageCount ? pageCount : 1}`
                            )
                        }
                    >

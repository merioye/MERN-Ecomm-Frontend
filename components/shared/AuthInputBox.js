...
const AuthInputBox = ({ label, placeholder, type, name, values, setValues, fromLoginPage }) => {
    ...
    return (
        <Box sx={style.container}>
            <InputLabel htmlFor={label} sx={style.label}>{label}</InputLabel>
            <input
                id={`${label}`} 
                style={type==='password' ? {...style.input, paddingRight: '50px' } : style.input }
                className='inputBox'
                ...
                onFocus={()=>name==='retypePassword' ? setIsInputFocused('true') : null}
            ...
            }
            {
                ...
                ?
                    showPassword
                    ?
                    <IconButton sx={style.visibilityToggleBtn} onClick={()=>setShowPassword(false)}>
                        <Visibility sx={{fontSize: '22px'}}/>
                    </IconButton>
                    :
                    <IconButton sx={style.visibilityToggleBtn} onClick={()=>setShowPassword(true)}>
                        <VisibilityOff sx={{fontSize: '22px', color: 'text.fadedSilver'}}/>
                    </IconButton>
                :
                null
            }

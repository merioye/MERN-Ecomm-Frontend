...
import { useTheme, Box, Grid, Typography, TextField, InputAdornment, Button, IconButton, ClickAwayListener } from "@mui/material";
...
const Navbar = () => {
    ...
    const fetchSuggestions = async(value)=>{
        ...
        try{
            const res = await axiosInstance.get(`${process.env.NEXT_PUBLIC_SERVER_BASE_URL}/api/products/search?search=${value.trim()}&page=0.5`);
            ...
            >
                ...
                >
                    <TextField
                        ...
                        InputProps={{
                            ...
                            startAdornment: (
                                <InputAdornment position="start">
                                    <SearchIcon sx={{ color: 'text.light', fontSize: '20px' }} />
                                </InputAdornment>
                            ),
                            endAdornment: (
                        ...
                        onKeyPress={(e)=> {
                            if(e.key === 'Enter') {
                                setTimeout(()=>{
                                    setShowSearchResults(false)
                                    router.push(`/search?productName=${e.target.value.trim()}`)
                                }, 100)
                            }
                        }}
                    />

                    {
                        ...
                        <ClickAwayListener onClickAway={()=>setShowSearchResults(false)}>
                            <Box 
                                sx={style.popup}
                                style={{...style.searchResults, boxShadow: theme.palette.boxShadow.nav }}
                            >
                                {
                                ...
                                <Box sx={style.itemsList}>
                                    {
                                        ...
                                        searchResults.map((product)=>{
                                            return <Link key={product._id} href={`/product/${product._id}`}>
                                                <a>
                                                    <Button sx={style.item} style={{ justifyContent: 'flex-start' }}>
                                                        ...
                                                        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', width: 'calc(100% - 84px)' }}>
                                                            <Typography variant='body1' sx={style.productName} mb='6px'>{product.name}</Typography>
                                                            <Typography variant='body1' sx={{...style.productName, color: 'bg.azureBlue' }}
                                                            >${product.isOnSale ? product.salePrice : product.regularPrice}</Typography>
                                                        </Box>
                                                    </Button>
                                                </a>
                                            </Link>
                                        ...
                                        :
                                        <Typography variant='body1' sx={style.item} style={{ textAlign: 'center', padding: '30px 0px' }}>No results matched!</Typography>
                                    }
                                    {
                                        searchResults.length
                                        ?
                                        <Button sx={style.viewAllBtn} onClick={redirect}>View all results &gt;</Button>
                                        :
                                        null
                                    }
                                </Box>
                ...
                >

                    {
                        ...
                        ?
                        <IconButton 
                            sx={{ height: '45px', width: '45px', padding: '0px' }}
                    ...
                    <NavProfilePopup profilePopup={profilePopup} setProfilePopup={setProfilePopup}/>
                </Grid>

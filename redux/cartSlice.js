...
const updateCartItems = (cartItems, product, type, dispatch)=>{
    ...
    if(type==='inc'){
        ...
        if(!isItemAlreadyPresent) updatedItems.unshift({ quantity: 1, product: product });
...
export function updateCart(count, setCount, product, type){
    return async function updateCartThunk(dispatch, getState){
        const { cartItems } = getState().cart;
        try{
            const { isUpdated, updatedCartItems } = updateCartItems(cartItems, product, type, dispatch);
            if(!isUpdated) return;

            let orderTotalAmount = 0;
            const dataToUpdateInBackend = updatedCartItems.map((p)=>{
                orderTotalAmount = orderTotalAmount+((p.product.isOnSale ? p.product.salePrice : p.product.regularPrice)*p.quantity);
                return { quantity: p.quantity, product: p.product._id };
            })

            const res = await axiosInstance.put('/api/carts/cart', { updatedProducts: dataToUpdateInBackend, orderTotalAmount });
            ...
            if(Boolean(res.data.isVoucherRemoved)){
                ...
                dispatch(showErrorAlert(`Voucher has been removed because minimum ${res.data.minimumAmountForVoucher} is required to avail this voucher`));
            }
...
export function applyVoucher(values, setShowLoader, setVoucherStatus, setDiscountPercentage){
    return async function applyVoucherThunk(dispatch){
        try{
            ...
            dispatch(setVoucher({ code: res.data.voucher.code, percentage: res.data.voucher.percentage }));
        }catch(e){
            console.log(e);
            setShowLoader(false);
